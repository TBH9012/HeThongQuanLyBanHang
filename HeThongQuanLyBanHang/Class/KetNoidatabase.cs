...
namespace HeThongQuanLyBanHang.Class
{
    internal class KetNoidatabase
    {
        ...
        public List<T> GetData<T>(Func<IQueryable<T>, IQueryable<T>> query) where T : class
        {
            try
            {
                return query(Con.Set<T>()).ToList();  
            }
            catch (Exception ex)
            {
        ...
        public static void RunSQL(string sql)
        {
            try
            {
                Con.Database.ExecuteSqlCommand(sql);  // Thực thi câu lệnh SQL
            }
            catch (Exception ex)
            {
        ...
        public static bool CheckKey(string sql)
        {
            try
            {
                return Con.Database.SqlQuery<int>(sql).FirstOrDefault() > 0;  // Kiểm tra khóa trùng
            }
            catch (Exception ex)
            {
        ...
        public static string GetFieldValues(string sql)
        {
            try
            {
                var result = Con.Database.SqlQuery<string>(sql).FirstOrDefault();
                return result ?? string.Empty; // Trả về chuỗi rỗng nếu không có kết quả
            }
            catch (Exception ex)
            {
                MessageBox.Show("Lỗi khi lấy giá trị: " + ex.Message);
                return string.Empty;
            }
        }
        public static void FillCombo(string sql, ComboBox cbo, string ma, string ten)
        {
            try
            {
                var data = Con.Database.SqlQuery<dynamic>(sql).ToList();
